<template>
  <div>
    <div class="home">
      <div class="row justify-center justify-md-start">
        <div class="col-xs-10 col-sm-6 col-md-4 col-lg-3">
          <router-link to="/random-string">
            <div class="app-item">
              <div class="icon">
                <svg t="1593334876779" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2265" width="64" height="64"><path d="M128 213.333333C128 166.4 166.4 128 213.333333 128L810.666667 128C857.6 128 896 166.4 896 213.333333L896 810.666667C896 857.6 857.6 896 810.666667 896L213.333333 896C165.973333 896 128 857.6 128 810.666667L128 213.333333M533.333333 469.333333 490.666667 469.333333C455.253333 469.333333 426.666667 440.746667 426.666667 405.333333 426.666667 369.92 455.253333 341.333333 490.666667 341.333333L533.333333 341.333333C568.746667 341.333333 597.333333 369.92 597.333333 405.333333L682.666667 405.333333C682.666667 322.986667 615.68 256 533.333333 256L490.666667 256C408.32 256 341.333333 322.986667 341.333333 405.333333 341.333333 487.68 408.32 554.666667 490.666667 554.666667L533.333333 554.666667C568.746667 554.666667 597.333333 583.253333 597.333333 618.666667 597.333333 654.08 568.746667 682.666667 533.333333 682.666667L490.666667 682.666667C455.253333 682.666667 426.666667 654.08 426.666667 618.666667L341.333333 618.666667C341.333333 701.013333 408.32 768 490.666667 768L533.333333 768C615.68 768 682.666667 701.013333 682.666667 618.666667 682.666667 536.32 615.68 469.333333 533.333333 469.333333Z" p-id="2266" fill="#1296db"></path></svg>
              </div>
              <div class="title">生成随机字符串</div>
              <div class="description">按照需求自定义生成字符串</div>
            </div>
          </router-link>
        </div>
        <div class="col-xs-10 col-sm-6 col-md-4 col-lg-3">
          <router-link to="/base-conversion">
            <div class="app-item">
              <div class="icon">
                <svg width="64" height="64" t="1595896394662" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2563"><path d="M512 512m-414 0a414 414 0 1 0 828 0 414 414 0 1 0-828 0Z" fill="#F0C48A" p-id="2564"></path><path d="M415.6 688.6h192.8v152.4h-192.8z" fill="#FD919E" p-id="2565"></path><path d="M536.2 692h72v137.2h-72zM331.4 841h361V928H331.4z" fill="#E8677D" p-id="2566"></path><path d="M955 688.6h-886c-31.6 0-57.2-25.6-57.2-57.2V153.2C11.8 121.6 37.4 96 69 96h886c31.6 0 57.2 25.6 57.2 57.2v478.2c0 31.6-25.6 57.2-57.2 57.2z" fill="#D3E6F8" p-id="2567"></path><path d="M944.4 688.6h-10.2V102h10.2c34 0 61.8 27.6 61.8 61.8v463.2c-0.2 34-27.8 61.6-61.8 61.6z" fill="#A4CFF2" p-id="2568"></path><path d="M78.4 171.6h867v441.2H78.4z" fill="#FFFFFF" p-id="2569"></path><path d="M868 174.6h72v438.4h-72z" fill="#D3E6F8" p-id="2570"></path><path d="M955 700.4h-886c-38 0-69-31-69-69V153.2c0-38 31-69 69-69h886c38 0 69 31 69 69v478.2c0 38-31 69-69 69zM69 107.6C44 107.6 23.4 128 23.4 153.2v478.2c0 25 20.4 45.6 45.6 45.6h886c25 0 45.6-20.4 45.6-45.6V153.2c0-25-20.4-45.6-45.6-45.6h-886z" fill="#4C4372" p-id="2571"></path><path d="M945.6 624.6H78.4c-6.4 0-11.8-5.2-11.8-11.8V171.6c0-6.4 5.2-11.8 11.8-11.8h867c6.4 0 11.8 5.2 11.8 11.8v441.2c0 6.6-5.2 11.8-11.6 11.8zM90.2 601.2h843.6V183.4H90.2v417.8zM608.4 852.6h-192.8c-6.4 0-11.8-5.2-11.8-11.8v-152.4c0-6.4 5.2-11.8 11.8-11.8h192.8c6.4 0 11.8 5.2 11.8 11.8v152.4c-0.2 6.6-5.4 11.8-11.8 11.8z m-181-23.4h169.2v-128.8h-169.2v128.8z" fill="#4C4372" p-id="2572"></path><path d="M692.6 939.8H331.4c-6.4 0-11.8-5.2-11.8-11.8v-87c0-6.4 5.2-11.8 11.8-11.8h361c6.4 0 11.8 5.2 11.8 11.8V928c0 6.6-5.2 11.8-11.6 11.8z m-349.4-23.4h337.6v-63.6H343.2v63.6zM460 296.4c0.2-19 5-36 14-49.2 8.6-13.2 22-22.8 38.2-22.8 15.8 0 29.4 9.6 38 22.8 8.8 13.2 13.8 30.2 13.8 49.2 0 18.6-5 36-13.8 49.2-8.6 13-22.2 22.8-38 22.8-16.2 0-29.6-9.8-38.2-22.8-9-13.4-13.8-30.6-14-49.2z m81.2 0c0-14.8-4.2-27.8-9.8-36.6-6-8.6-12.8-12.8-19.2-12.6-6.4-0.2-13.4 4-19.6 12.6-5.8 8.8-9.8 21.8-9.8 36.6 0 14.6 4.2 27.8 9.8 36.4 6.2 8.8 13.2 12.8 19.6 12.8 6.4 0 13.2-4 19.2-12.8 5.6-8.8 9.8-22 9.8-36.4zM536.2 414.8V536c0 6.4-5.4 11.4-11.6 11.4-6.4 0-11.4-5-11.4-11.4v-101l-8.2 4.8c-1.8 1.2-4 1.6-5.8 1.6-4 0-7.6-2-9.6-5.8-3.4-5.2-1.6-12.4 3.6-15.4l25.2-15c0.2-0.2 0.6-0.2 0.8-0.4 0.4 0 0.6-0.4 1-0.6 0.4 0 0.8-0.2 1.2-0.2 0.2-0.2 0.6-0.2 0.8-0.2 0.4-0.2 1-0.2 1.4-0.2h2.2c0.2 0.2 0.6 0.2 0.8 0.2 0.6 0 0.8 0.2 1.2 0.4 0.2 0 0.6 0 0.8 0.2 0.6 0.4 0.8 0.6 1.2 0.6 0.2 0.2 0.4 0.4 0.8 0.6 0.2 0.2 0.6 0.4 0.8 0.6 0.4 0.2 0.6 0.6 0.8 0.8 0.2 0.2 0.4 0.4 0.6 0.8l0.8 0.8 0.2 0.4c0 0.2 0.2 0.4 0.2 0.6 0.2 0.4 0.4 0.6 0.4 1 0.2 0.4 0.2 0.8 0.4 1 0 0.4 0.2 0.8 0.2 1 0 0.6 0 1 0.4 1.4 0.8 0.4 0.8 0.6 0.8 0.8zM339.8 235.6v121.2c0 6.4-5.4 11.4-11.6 11.4-6.4 0-11.4-5-11.4-11.4v-101l-8.2 4.8c-1.8 1.2-4 1.6-5.8 1.6-4 0-7.6-2-9.6-5.8-3.4-5.2-1.6-12.4 3.6-15.4L322 226c0.2-0.2 0.6-0.2 0.8-0.4 0.4 0 0.6-0.4 1-0.6 0.4 0 0.8-0.2 1.2-0.2 0.2-0.2 0.6-0.2 0.8-0.2 0.4-0.2 1-0.2 1.4-0.2h2.2c0.2 0.2 0.6 0.2 0.8 0.2 0.6 0 0.8 0.2 1.2 0.4 0.2 0 0.6 0 0.8 0.2 0.6 0.4 0.8 0.6 1.2 0.6 0.2 0.2 0.4 0.4 0.8 0.6 0.2 0.2 0.6 0.4 0.8 0.6 0.4 0.2 0.6 0.6 0.8 0.8 0.2 0.2 0.4 0.4 0.6 0.8l0.8 0.8 0.2 0.4c0 0.2 0.2 0.4 0.2 0.6 0.2 0.4 0.4 0.6 0.4 1 0.2 0.4 0.2 0.8 0.4 1 0 0.4 0.2 0.8 0.2 1 0 0.6 0 1 0.4 1.4 0.8 0.4 0.8 0.6 0.8 0.8zM732.4 235.6v121.2c0 6.4-5.4 11.4-11.6 11.4-6.4 0-11.2-5-11.2-11.4v-101l-8.2 4.8c-1.8 1.2-4 1.6-5.8 1.6-4 0-7.6-2-9.6-5.8-3.4-5.2-1.6-12.4 3.6-15.4l25.2-15c0.2-0.2 0.6-0.2 0.8-0.4 0.4 0 0.6-0.4 1-0.6 0.4 0 0.8-0.2 1.2-0.2 0.2-0.2 0.6-0.2 0.8-0.2 0.4-0.2 1-0.2 1.4-0.2h2.2c0.2 0.2 0.6 0.2 0.8 0.2 0.6 0 0.8 0.2 1.2 0.4 0.2 0 0.6 0 0.8 0.2 0.6 0.4 0.8 0.6 1.2 0.6 0.2 0.2 0.4 0.4 0.8 0.6 0.2 0.2 0.6 0.4 0.8 0.6 0.4 0.2 0.6 0.6 0.8 0.8 0.2 0.2 0.4 0.4 0.6 0.8l0.8 0.8 0.2 0.4c0 0.2 0.2 0.4 0.2 0.6 0.2 0.4 0.4 0.6 0.4 1 0.2 0.4 0.2 0.8 0.4 1 0 0.4 0.2 0.8 0.2 1 0 0.6 0 1 0.4 1.4 0.6 0.4 0.6 0.6 0.6 0.8zM263.8 475.4c0.2-19 5-36 14-49.2 8.6-13.2 22-22.8 38.2-22.8 15.8 0 29.4 9.6 38 22.8 8.8 13.2 13.8 30.2 13.8 49.2 0 18.6-5 36-13.8 49.2-8.6 13-22.2 22.8-38 22.8-16.2 0-29.6-9.8-38.2-22.8-9-13.2-13.8-30.4-14-49.2z m81.2 0c0-14.8-4.2-27.8-9.8-36.6-6-8.6-12.8-12.8-19.2-12.6-6.4-0.2-13.4 4-19.6 12.6-5.8 8.8-9.8 21.8-9.8 36.6 0 14.6 4.2 27.8 9.8 36.4 6.2 8.8 13.2 12.8 19.6 12.8 6.4 0 13.2-4 19.2-12.8 5.6-8.6 9.8-21.8 9.8-36.4zM656.2 475.4c0.2-19 5-36 14-49.2 8.6-13.2 22-22.8 38.2-22.8 15.8 0 29.4 9.6 38 22.8 8.8 13.2 13.8 30.2 13.8 49.2 0 18.6-5 36-13.8 49.2-8.6 13-22.2 22.8-38 22.8-16.2 0-29.6-9.8-38.2-22.8-9-13.2-13.8-30.4-14-49.2z m81.2 0c0-14.8-4.2-27.8-9.8-36.6-6-8.6-12.8-12.8-19.2-12.6-6.4-0.2-13.4 4-19.6 12.6-5.8 8.8-9.8 21.8-9.8 36.6 0 14.6 4.2 27.8 9.8 36.4 6.2 8.8 13.2 12.8 19.6 12.8s13.2-4 19.2-12.8c5.8-8.6 9.8-21.8 9.8-36.4z" fill="#4C4372" p-id="2573"></path></svg>
              </div>
              <div class="title">进制转换</div>
              <div class="description">基本的进制转换</div>
            </div>
          </router-link>
        </div>
        <div class="col-xs-10 col-sm-6 col-md-4 col-lg-3">
          <router-link to="/image-conversion">
            <div class="app-item">
              <div class="icon">
                <svg width="64" height="64" t="1595896419347" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2716"><path d="M880 192h-736C117.44 192 96 213.44 96 240v544c0 26.56 21.44 48 48 48h736c26.56 0 48-21.44 48-48v-544c0-26.56-21.44-48-48-48z m16 406.08v185.92c0 8.96-7.04 16-16 16h-736c-8.96 0-16-7.04-16-16v-24-32l329.92-217.92 97.28 71.36 167.68-135.36L896 566.08v32z m0-73.28l-154.88-104.96c-5.76-3.84-11.84-5.76-17.952-5.76-6.048 0-12.48 1.92-17.92 5.76l-150.4 120.32-80.64-56.96a31.36 31.36 0 0 0-17.92-5.76c-6.08 0-12.16 1.92-17.92 5.76L128 689.28V240c0-8.96 7.04-16 16-16h736c8.96 0 16 7.04 16 16v284.8z" fill="#2D2220" p-id="2717"></path><path d="M416 352c0 29.76-20.48 54.72-48 61.76-27.52-7.04-48-32-48-61.76s20.48-54.72 48-61.76c27.52 7.04 48 32 48 61.76z" fill="#FFD561" p-id="2718"></path><path d="M896 598.08v185.92c0 8.96-7.04 16-16 16h-736c-8.96 0-16-7.04-16-16v-24l329.92-217.92 97.28 71.36 167.68-135.36L896 598.08z" fill="#5FFFBA" p-id="2719"></path><path d="M352 288c35.296 0 64 28.704 64 64s-28.704 64-64 64-64-28.704-64-64 28.704-64 64-64m0-32a96 96 0 1 0 0 192 96 96 0 0 0 0-192z" fill="#2D2220" p-id="2720"></path></svg>
              </div>
              <div class="title">图片转换</div>
              <div class="description">图片格式在线转换</div>
            </div>
          </router-link>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
  export default {

  }
</script>

<style type="scss">
  .tool-item {
    max-width: 230px !important;
    padding: 15px;
  }
  a {
    text-decoration: none;
    color: black;
  }
  .home {
    margin: 25px;
  }
  .app-item {
    box-shadow: 0 0 35px 0 rgba(0,0,0,.2);
    padding: 20px 10px;
    margin: 15px auto;
    max-width: 230px;
  }
  .icon {
    text-align: center;
  }
  .title {
    text-align: center;
    margin-bottom: 10px;
    font-size: 16px;
  }
  .description {
    color: gray;
    text-align: center;
    font-size: 14px;
  }
</style>
